...
#
#
# class CTNormalizationWithMetalMask(ImageNormalization):
#     leaves_pixels_outside_mask_at_zero_if_use_mask_for_norm_is_true = True
#
#     def run(self, image: np.ndarray, seg: np.ndarray = None) -> np.ndarray:
#         assert self.intensityproperties is not None, "This normalization requires intensity properties"
#         image = image.astype(self.target_dtype)
#
#         # 生成金属掩膜 (CT值>2500HU)
#         metal_mask = (image > 2500)
#
#         # 创建组织掩膜 (排除金属区域)
#         tissue_mask = np.logical_and(image > -200, np.logical_not(metal_mask))
#
...
#             lower_bound = np.percentile(tissue_pixels, 0.5)
#             upper_bound = np.percentile(tissue_pixels, 99.5)
#         else:
#             # 回退到全局统计
#             mean_intensity = self.intensityproperties['mean']
#             std_intensity = self.intensityproperties['std']
