...
# from .mar_correction import MultiscaleSparseMARNet
#
# class DefaultPreprocessor(object):
#     def __init__(self, verbose: bool = True, mar_enabled: bool = True):
#         self.verbose = verbose
#         """
#         Everything we need is in the plans. Those are given when run() is called
#         """
#         self.mar_enabled = mar_enabled
#         if mar_enabled:
#             self.mar_net = MultiscaleSparseMARNet().eval()  # 预训练MAR模型
#             self.mar_net.load_state_dict(torch.load('mar_model_weights.pth'))
#
...
#
#         if self.mar_enabled and 'CT' in str(dataset_json.get('modality', '')):
#             if self.verbose:
#                 print("Applying metal artifact correction...")
#
#             # 生成金属掩膜 (阈值>2500HU)
#             metal_mask = (data[0] > 2500).astype(np.float32)
#
#             # 转换为Tensor格式
#             data_tensor = torch.from_numpy(data).float()
#             metal_tensor = torch.from_numpy(metal_mask).unsqueeze(0).float()
#
#             # 执行MAR校正
#             with torch.no_grad():
#                 corrected = self.mar_net(data_tensor, metal_tensor)
#
#             # 更新数据并保留金属区域
#             data = corrected.numpy()
#             data[0][metal_mask > 0] = 2500  # 保留金属区域原始值
#
#             if self.verbose:
#                 print("Metal artifact correction completed")
#
#         # normalize
