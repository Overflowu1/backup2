...
class nnUNetPredictor(object):
    ...
    def _internal_predict_sliding_window_return_logits(self, data, slicers, do_on_device: bool = True):
        ...
        if self.use_gaussian:
            ...
            gaussian = compute_gaussian(
                ...
                sigma_scale=1. / 8,
                value_scaling_factor=5,  # 原来是 10，必要时可再降
        ...
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            ...
            # 网络在 autocast 下可能产出半精度，这里强制转为 float32 再参与累计
            pred = self._internal_maybe_mirror_and_predict(workon)[0].to(results_device).to(torch.float32)
            ...
            if self.use_gaussian:
                ...
                n_predictions[sl[1:]] += 1.0

        predicted_logits /= n_predictions

        # 万一仍有数值异常（基本不会了），这里兜底一下
        if torch.any(torch.isinf(predicted_logits)) or torch.any(torch.isnan(predicted_logits)):
            predicted_logits = torch.nan_to_num(predicted_logits, posinf=0.0, neginf=0.0)
...
def predict_entry_point():
    ...
    parser = argparse.ArgumentParser(description='Use this to run inference with nnU-Net. This function is used when '
                                                 ...
                                                 'model. This is useful when the nnunet environment variables '
                                                 '(nnUNet_results) are not set.')
    parser.add_argument('-i', type=str, required=True,default='/home/ps/wyc/nnUNet/DATA/nnUNet_raw/Dataset011_4pelvis/imagesTs1',
                        help='input folder. Remember to use the correct channel numberings for your files (_0000 etc). '
                             'File endings must be the same as the training dataset!')
    parser.add_argument('-o', type=str, required=True,default='/home/ps/wyc/nnUNet/DATA/nnUNet_raw/Dataset011_4pelvis/imagesTs3',
                        help='Output folder. If it does not exist it will be created. Predicted segmentations will '
                             'have the same name as their source images.')
    parser.add_argument('-d', type=str, required=True,default='11',
                        help='Dataset with which you would like to predict. You can specify either dataset name or id')
    parser.add_argument('-p', type=str, required=False, default='nnUNetPlans',
                        help='Plans identifier. Specify the plans in which the desired configuration is located. '
                             'Default: nnUNetPlans')
    parser.add_argument('-tr', type=str, required=False, default='nnUNetTrainerMiFDeU',
                        help='What nnU-Net trainer class was used for training? Default: nnUNetTrainer')
    parser.add_argument('-c', type=str, required=True,default='3d_lowres',
                        help='nnU-Net configuration that should be used for prediction. Config must be located '
                             'in the plans specified with -p')
    parser.add_argument('-f', nargs='+', type=str, required=False, default='3',
                        help='Specify the folds of the trained model that should be used for prediction. '
                             'Default: (0, 1, 2, 3, 4)')
    parser.add_argument('-step_size', type=float, required=False, default=0.5,
                        help='Step size for sliding window prediction. The larger it is the faster but less accurate '
...
if __name__ == '__main__':
    ...
    # predictor.initialize_from_trained_model_folder(
    #     join(nnUNet_results, 'Dataset112_fra/nnUNetTrainerSwinUNETR__nnUNetPlans__3d_lowres/'),
    #     use_folds=(0, ),
    #     checkpoint_name='checkpoint_best.pth',
    # )
    predictor.initialize_from_trained_model_folder(
        '/mnt/data/DATA/nnUNet_results/Dataset011_4pelvis/nnUNetTrainer__nnUNetPlans__3d_lowres',
        use_folds=(0,),
        checkpoint_name='checkpoint_final.pth',
    )
    # predictor.initialize_from_trained_model_folder(
    #     '//mnt/data/DATA/nnUNet_results/Dataset123_fra/ULstmBotTrainer__nnUNetPlans__3d_lowres',
    #     use_folds=(0,),
    #     checkpoint_name='checkpoint_final.pth',
    # )nnUNetv2_predict -i /mnt/data/DATA/227/intra -o /mnt/data/DATA/227/simsegr -d 31 -c 3d_lowres
    predictor.predict_from_files(join('/mnt/data/DATA/416'),
                                 join('/mnt/data/DATA/416/net4'),
                                 save_probabilities=True, overwrite=True,
                                  num_processes_preprocessing=2, num_processes_segmentation_export=2,
                                 folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)
    # predictor.predict_from_files(join('//mnt/data/DATA/zjyData/VISUAL/nnunet/CT'),
    #                              join('/mnt/data/DATA/zjyData/VISUAL/nnunet/CT/predict/'),
    #                              save_probabilities=True, overwrite=True,
    #                              num_processes_preprocessing=2, num_processes_segmentation_export=2,
    #                              folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)
