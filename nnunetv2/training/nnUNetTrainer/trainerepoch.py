...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_CE_and_Surface_loss
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        if self.is_ddp:  # implicitly it's clear that we use cuda in this case
            print('MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMutil')
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss = self._build_loss(epoch=self.current_epoch)
    ...
    def _build_loss(self,epoch):
        if self.label_manager.has_regions:
            ...
            loss = DC_and_CE_and_Surface_loss(
                {'batch_dice': self.configuration_manager.batch_dice, 'smooth': 1e-5, 'do_bg': False,
                 'ddp': self.is_ddp},
                {},
                {'idc': [0]},  # 假设只对第一个类别计算 SurfaceLoss
                weight_ce=1, weight_dice=1, weight_surface=0.5,
        ...
        #      'min_thick': 1},
        #     weight_ce=1, weight_dice=1, weight_ti=bou, ignore_label=self.label_manager.ignore_label,
        #     dice_class=MemoryEfficientSoftDiceLoss)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l = self.loss(output, target,epoch=self.current_epoch)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data)
            del data
            l = self.loss(output, target,epoch=self
                        .current_epoch)
