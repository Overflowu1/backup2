...
class UNetResDecoder(nn.Module):
    def __init__(self,
        ...
        if isinstance(n_conv_per_stage, int):
            n_conv_per_stage = [n_conv_per_stage] * (n_stages_encoder - 1)
        assert len(
            n_conv_per_stage) == n_stages_encoder - 1, "n_conv_per_stage must have as many entries as we have resolution stages - 1"

        stages = []
        upsample_layers = []
        seg_layers = []
        asfeb_modules = []  # 新增 ASFEB 模块列表
        ...
        for s in range(1, n_stages_encoder):
            ...
            # 添加 ASFEB 模块
            asfeb_modules.append(ASFEB(input_features_skip, input_features_skip))
        ...
        self.upsample_layers = nn.ModuleList(upsample_layers)
        self.seg_layers = nn.ModuleList(seg_layers)
        self.asfeb_modules = nn.ModuleList(asfeb_modules)  # 转换为 ModuleList
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            x = self.upsample_layers[s](lres_input)
            # 使用 ASFEB 模块增强跳跃连接特征
            skip_features = self.asfeb_modules[s](skips[-(s + 2)])
