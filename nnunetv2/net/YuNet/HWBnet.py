...
from nnunetv2.net.YuNet.HWB import HWB


class HWBPlainConvUNet(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
    ...
    def __init__(self,
        ...
        self.convs = nn.Sequential(
            ...
            *[
            ...
            HWB(conv_op, output_channels[-1], output_channels[-1], kernel_size, conv_bias,nn.LeakyReLU(inplace=True))
        )
...
if __name__ == '__main__':
    data = torch.rand((1, 4, 512, 512)).to('cuda:0')
    relu_kwargs = {
        'inplace': True  # 这里示例中设置了 inplace=True，表示原地操作
    }

    model = HWBPlainConvUNet(4, 8, (32, 64, 125, 256, 512, 512, 512, 512), nn.Conv2d, 3, (1, 2, 2, 2, 2, 2, 2, 2),
                            (2, 2, 2, 2, 2, 2, 2, 2), 5,
                            (2, 2, 2, 2, 2, 2, 2), False, nn.BatchNorm2d, None, None, None, nn.ReLU, relu_kwargs,
                            deep_supervision=True).to('cuda:0')
