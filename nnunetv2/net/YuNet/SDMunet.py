...
from nnunetv2.net.YuNet.SDM3d import SDM


class SDMPlainConvUNet(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
...
class CAUNetDecoder(nn.Module):
    def __init__(self,
        ...
        in_channels_list = [320, 256, 128, 64, 32]
        guidance_channels_list = [320, 256, 128, 64, 32]
        self.sdm_modules = nn.ModuleList([SDM(in_channels_list[i], guidance_channels_list[i]) for i in range(len(in_channels_list))])
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            # 如果定义了 SDM 模块，则传递 skip 和 x 特征图
            if self.sdm_modules is not None:
                x = self.sdm_modules[s](skip_feature, x)
...
if __name__ == '__main__':
    ...
    relu_kwargs = {
        'inplace': False  # 这里示例中设置了 inplace=True，表示原地操作
    }

    model = SDMPlainConvUNet(4, 6, (32, 64, 125, 256, 320, 320), nn.Conv3d, 3, (1, 2, 2, 2, 2, 2), (2, 2, 2, 2, 2, 2), 6,
                                (2, 2, 2, 2, 2), False, nn.BatchNorm3d, None, None, None, nn.LeakyReLU, relu_kwargs,deep_supervision=True)
    out = model(data)
    print(out[0].shape)
