...
if __name__ == "__main__":
    # 输入数据
    B, S, dim = 8, 16, 32  # batch size, sequence length, feature dimension
    x = torch.randn(B, S, dim)

    # 测试 DropPath 模块
    print("Testing DropPath module...")
    drop_path = DropPath(nn.Linear(dim, dim), drop_prob=0.2)
    y_drop_path = drop_path(x)
    print("DropPath output shape:", y_drop_path.shape)

    # 测试 interpolate_sincos 函数
    print("\nTesting interpolate_sincos function...")
    embed = torch.randn(1, 8, 8, dim)
    seqlens = (16, 16)
    interpolated_embed = interpolate_sincos(embed, seqlens, mode="bicubic")
    print("Interpolated embed shape:", interpolated_embed.shape)

    # 测试 MatrixLSTMCell 模块
    print("\nTesting MatrixLSTMCell module...")
    matrix_lstm_cell = MatrixLSTMCell(dim=dim, num_heads=4)
    q = torch.randn(B, S, dim)
    k = torch.randn(B, S, dim)
    v = torch.randn(B, S, dim)
    h_tilde_state = matrix_lstm_cell(q, k, v)
    print("MatrixLSTMCell output shape:", h_tilde_state.shape)

    # 测试 ViLLayer 模块
    print("\nTesting ViLLayer module...")
    vil_layer = ViLLayer(dim=dim, direction=SequenceTraversal.ROWWISE_FROM_TOP_LEFT)
    y_vil_layer = vil_layer(x)
    print("ViLLayer output shape:", y_vil_layer.shape)

    # 测试 ViLBlock 模块
    print("\nTesting ViLBlock module...")
    vil_block = ViLBlock(dim=dim, direction=SequenceTraversal.ROWWISE_FROM_TOP_LEFT)
    y_vil_block = vil_block(x)
    print("ViLBlock output shape:", y_vil_block.shape)
