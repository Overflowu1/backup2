...
from nnunetv2.net.to_torch import to_cuda
...
class BTI_Loss(torch.nn.Module):
    ...
    def binary_topological_interaction_module(self, P):
        ...
        for ind, interaction in enumerate(self.interaction_list):
            ...
            # label_A = to_cuda(torch.tensor(label_A).clone().detach())
            # label_C = to_cuda(torch.tensor(label_C).clone().detach())
    ...
    def compute_topological_loss(self, critical_voxels_map, ground_truth):
        # Initialize topological loss
        topological_loss = 0

        # Extract individual bone masks based on ground truth
        sacrum_mask = (ground_truth == 1).float()
        left_hip_mask = (ground_truth == 2).float()
        right_hip_mask = (ground_truth == 3).float()
        coccyx_mask = (ground_truth == 4).float()

        # Define topological rules
        # 1. The sacrum (1) should be connected to the coccyx (4) and not to the hips (2 and 3)
        sacrum_to_coccyx = sacrum_mask * critical_voxels_map * (1 - (left_hip_mask + right_hip_mask))
        topological_loss += sacrum_to_coccyx.sum()

        # 2. The hips (2 and 3) should not be connected to the coccyx (4)
        hips_to_coccyx = (left_hip_mask + right_hip_mask) * coccyx_mask * critical_voxels_map
        topological_loss += hips_to_coccyx.sum()

        # 3. The hips (2 and 3) should be connected to the sacrum (1)
        hips_to_sacrum = (left_hip_mask + right_hip_mask) * sacrum_mask * critical_voxels_map
        topological_loss += hips_to_sacrum.sum()

        # 4. Left hip (2) and right hip (3) should not be connected
        hips_not_connected = left_hip_mask * right_hip_mask * critical_voxels_map
        topological_loss += hips_not_connected.sum()
    ...
    def forward(self, x, y):
        ...
        #topo_loss = self.compute_topological_loss(critical_voxels_map=critical_voxels_map, ground_truth=y)
        result_loss = ce_loss_value
        return result_loss
