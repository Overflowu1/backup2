...
from nnunetv2.net.MiFDeU.network_architecture.torch_nn import BasicConv, batched_index_select, \
    act_layer
from nnunetv2.net.MiFDeU.network_architecture.torch_edge import DenseDilatedKnnGraph
from nnunetv2.net.MiFDeU.network_architecture.pos_embed import get_2d_relative_pos_embed, \
...
class MiFDeU_Encoder(nn.Module):
    def __init__(self,
                 input_channels: int,
                 patch_size: List[int],
                 n_stages: int,
        ...
        for s in range(n_stages):
            ...
            if s < self.n_conv_stages:
                stage = StackedResidualBlocks(n_blocks_per_stage[s], conv_op, input_channels, features_per_stage[s],
                ...
                stage = nn.Sequential(
                    StackedResidualBlocks(n_blocks_per_stage[s] - 1, conv_op, input_channels, features_per_stage[s],
                                          ...
                                          squeeze_excitation=squeeze_excitation,
                                          squeeze_excitation_reduction_ratio=squeeze_excitation_reduction_ratio),
                    SwinGNNBlocks(features_per_stage[s], img_shape_list[s], s - self.n_conv_stages, opt=self.opt,
                                  conv_op=conv_op,
...
class MiFDeU_Decoder(nn.Module):
    def __init__(self,
                 encoder: Union[PlainConvEncoder, ResidualEncoder, MiFDeU_Encoder],
                 patch_size: List[int],
                 strides: Union[int, List[int], Tuple[int, ...]],
                 num_classes: int,
            ...
            if s < (n_stages_encoder - self.no_pool_gnn_stage_num):
                stages.append(nn.Sequential(
                    ...
                    PoolGNNBlocks(input_features_skip, img_shape_list[n_stages_encoder - (s + 1)],
                                  n_stages_encoder - self.no_pool_gnn_stage_num - (s + 1), self.no_pool_gnn_stage_num,
                                  opt=self.opt, conv_op=encoder.conv_op,
...
def window_reverse(windows, window_size, size_tuple):
    ...
    if len(windows.shape) == 4:
        ...
        raise NotImplementedError('len(x.shape) [%d] is equal to 4 or 5' )
...
class SwinGNNBlocks(nn.Module):
    def __init__(self, channels, img_shape, index, opt=None, conv_op=nn.Conv3d, norm_op=nn.BatchNorm3d,
                 norm_op_kwargs=None,
...
class PoolGNNBlocks(nn.Module):
    def __init__(self, channels, img_shape, index, stage_num, opt=None, conv_op=nn.Conv3d, norm_op=nn.BatchNorm3d,
                 norm_op_kwargs=None,
                 dropout_op=nn.Dropout3d, **kwargs):
        ...
        for j in range(blocks_num_list[index]):
            ...
            blocks.append(nn.Sequential(
                PoolGrapher(channels, img_shape, k_list[i + stage_num], min(idx // 4 + 1, max_dilation), conv, act,
                            norm,
                            bias, stochastic, epsilon, reduce_ratios[i + stage_num], n=n_size_list[i + stage_num],
                            drop_path=dpr[idx],
                            relative_pos=True, conv_op=conv_op, norm_op=norm_op, norm_op_kwargs=norm_op_kwargs,
