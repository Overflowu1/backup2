...
from nnunetv2.net.MiFDeU.network_architecture.nd_softmax import softmax_helper
...
class SwinGrapher(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.conv_op == nn.Conv2d:
            ...
            assert torch.all(torch.tensor(S).eq(s)) and torch.all(torch.tensor(H).eq(h)) and torch.all(
                torch.tensor(W).eq(w)), "input feature has wrong size"
        else:
            raise NotImplementedError('conv operation [%s] is not found' % self.conv_op)
...
class Efficient_ViG_blocks(nn.Module):
    def __init__(self, channels, img_shape, index, conv_layer_d_num, opt=None, conv_op=nn.Conv3d,
        ...
        for j in range(blocks_num_list[index]):
            ...
            blocks.append(
                nn.Sequential(PoolGrapher(channels, img_shape, k[i], min(idx // 4 + 1, max_dilation), conv, act, norm,
                                          bias, stochastic, epsilon, reduce_ratios[i], n=n_size_list[i + 2],
                                          drop_path=dpr[idx],
                                          relative_pos=True, conv_op=conv_op, norm_op=norm_op,
                                          norm_op_kwargs=norm_op_kwargs, dropout_op=dropout_op,
                                          img_min_shape=img_min_shape),
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, patch_size, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            if d < conv_layer_d_num:
                ...
                self.conv_blocks_context.append(nn.Sequential(
                    ...
                    Efficient_ViG_blocks(output_features, img_shape_list[d], d - conv_layer_d_num, conv_layer_d_num,
                                         opt=self.opt, conv_op=self.conv_op,
        ...
        self.conv_blocks_context.append(nn.Sequential(
            StackedConvLayers(input_features, output_features, num_conv_per_stage - 1, self.conv_op, self.conv_kwargs,
                              ...
                              self.nonlin_kwargs, first_stride, basic_block=basic_block),
            Efficient_ViG_blocks(output_features, img_shape_list[d + 1], d - conv_layer_d_num + 1, conv_layer_d_num,
                                 opt=self.opt, conv_op=self.conv_op,
        ...
        for u in range(num_pool):
            ...
            if u < (num_pool - conv_layer_d_num):
                self.conv_blocks_localization.append(nn.Sequential(
                    ...
                    Efficient_ViG_blocks(nfeatures_from_skip, img_shape_list[num_pool - (u + 1)],
                                         num_pool - conv_layer_d_num - (u + 1), conv_layer_d_num, opt=self.opt,
