...
from nnunetv2.training.nnUNetTrainer.BTCV import cross_attention

__all__ = [
    "SwinUNETR",
]

from nnunetv2.training.nnUNetTrainer.BTCV import view_ops
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        x = torch.rand((1, 3, 128, 128, 128), device=cuda0)
        y = torch.rand((1, 3, 128, 128, 128), device=cuda0)
        ...
        print(pred.shape)
        target = torch.randint(0, 4, (2, 1, 128, 128, 128), device=cuda0)
        print(target.shape)
        # z = my_get_dice_loss(preds=y, target=y2)
        # pred = torch.softmax(out_list, dim=1)
        dice_loss = DiceCELoss(
            to_onehot_y=True, softmax=True, squared_pred=True, smooth_nr=1e-5, smooth_dr=1e-5
        )
        # inter = (pred * target)
        # union = (pred + target)
