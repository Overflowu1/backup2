...
class Mlp(nn.Module):
    def __init__(self, dim, drop_path=0.2, layer_scale_init_value=0.7):
        super().__init__()
        self.dwconv = nn.Conv3d(dim, dim, kernel_size=7, padding=3, groups=dim)  # depthwise conv 7,3  5,2  3,1
        self.norm = nn.LayerNorm(dim, eps=1e-6)
        self.pwconv1 = nn.Conv3d(dim, 4 * dim, kernel_size=1)
        self.act = nn.GELU()
        self.pwconv2 = nn.Conv3d(4 * dim, dim, kernel_size=1)  # nn.Linear(4 * dim, dim)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.proj = nn.Sequential(nn.Conv3d(dim, dim, kernel_size=3, padding=1, stride=1, bias=False, groups=dim), nn.GELU())
    ...
    def _build_projection(self, dim_in, kernel_size=3, stride=1, padding=1):
        proj = nn.Sequential(
            nn.Conv3d(dim_in, dim_in, kernel_size, padding=padding, stride=stride, bias=False, groups=dim_in),
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = nn.Sequential(nn.Conv3d(in_chans, embed_dim // 2, kernel_size=3, stride=1, padding=1), nn.GELU(),
                                  nn.Conv3d(embed_dim // 2, embed_dim // 2, kernel_size=3, stride=2, padding=1),
                                  nn.GELU(),
                                  Rearrange('b c h w -> b h w c'),
                                  norm_layer(embed_dim // 2),
                                  Rearrange('b h w c -> b c h w'),
                                  nn.Conv3d(embed_dim // 2, embed_dim, kernel_size=3, stride=1, padding=1), nn.GELU(),
                                  nn.Conv3d(embed_dim, embed_dim, kernel_size=3, stride=2, padding=1), nn.GELU())
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            concat_cov = self.up = nn.Sequential(Rearrange('b (h w) c -> b c h w', h=patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)), w=patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
                                                 nn.Conv3d(2 * int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                           int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                           kernel_size=3, stride=1, padding=1), nn.GELU(),
                                                 nn.Conv3d(int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                           int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                           kernel_size=3, stride=1, padding=1), nn.GELU(),
                                                 nn.Dropout(p=0.2),
